...
class CheckpointManager:
    def __init__(
        ...
        self.save_epoch_checkpoints = getattr(ckpt_config, 'save_epoch_checkpoints', False)
        self.begin_time = 0
        self.time_sync_work = None
        self.time_sync_result = None
        self.pg = dist.new_group(backend="gloo")
        
        # Epoch tracking for epoch-based checkpointing
        self.current_epoch = 0
        self.steps_per_epoch = 0
        self.last_epoch_checkpoint_step = 0
    ...
    def _create_checkpoint_id(self, step: int, checkpoint_type: str = "step") -> str:
        if checkpoint_type == "epoch":
            epoch_num = step // self.steps_per_epoch if self.steps_per_epoch > 0 else 0
            return os.path.join(self.folder, f"epoch-{epoch_num}-step-{step}")
        else:
    ...
    def set_epoch_info(self, steps_per_epoch: int) -> None:
        """Set epoch information for epoch-based checkpointing."""
        self.steps_per_epoch = steps_per_epoch
        logger.info(f"Set steps per epoch: {steps_per_epoch}")

    def _should_save_epoch_checkpoint(self, curr_step: int) -> bool:
        """Check if we should save an epoch checkpoint."""
        if not self.save_epoch_checkpoints or self.steps_per_epoch == 0:
            return False
        
        # Calculate current epoch
        current_epoch = curr_step // self.steps_per_epoch
        
        # Save checkpoint if we've completed a new epoch
        if current_epoch > self.current_epoch and curr_step > 0:
            self.current_epoch = current_epoch
            self.last_epoch_checkpoint_step = curr_step
            logger.info(f"Epoch {current_epoch} completed at step {curr_step}")
            return True
    ...
    def save(self, curr_step: int, force: bool = False) -> None:
        ...
        """
        # Check for epoch-based checkpointing first
        if self._should_save_epoch_checkpoint(curr_step):
            self._save_checkpoint(curr_step, checkpoint_type="epoch")
        
        # Then check for regular interval-based checkpointing
        if not self._should_save(curr_step, force):
            return

        self._save_checkpoint(curr_step, checkpoint_type="interval", force=force)

    def _save_checkpoint(self, curr_step: int, checkpoint_type: str = "interval", force: bool = False) -> None:
        """Internal method to save checkpoint."""
        begin = time.monotonic()
        checkpoint_id = self._create_checkpoint_id(curr_step, checkpoint_type)
        self._async_wait()
        if force:
        ...
        logger.info(
            f"Finished saving the {checkpoint_type} checkpoint (or staging if async is enabled) "
