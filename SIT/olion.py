...
def apply_double_momentum(grad, momentum_buffer, momentum, momentum_2, nesterov):
    """
    OLion's double momentum update.
    First updates momentum_buffer with momentum_2, then applies momentum with nesterov.
    """
...
def olion_update(grad, momentum_buffer, momentum, momentum_2, ns_steps, nesterov, rms_scale, use_scale, eps):
    """
    OLion update function with double momentum and sign operation.
    """
    # Apply double momentum
    update = apply_double_momentum(grad, momentum_buffer, momentum, momentum_2, nesterov)

    # Newton-Schulz orthogonalization
    update = fast_newtonschulz(update, steps=ns_steps)

    # Sign operation (OLion-specific)
    update = torch.sign(update)

    # Optional scaling
    if use_scale:
        scale = (min(update.size(-2), update.size(-1))) ** 0.5 / (update.norm() + eps)
        update = update * scale

    # Apply RMS scaling if enabled
...
class Fsdp1dWork:
    ...
    def start(self):
        # Apply double momentum for OLion
        self.param.grad = apply_double_momentum(
            self.param.grad,
            self.state["momentum_buffer"],
            self.group["momentum"],
            self.group["momentum_2"],
            self.group["nesterov"],
        )
    ...
    def finish(self):
        ...
        if rank == dest_rank:
            ...
            # Apply OLion update: NS orthogonalization + sign
            g_full_block = fast_newtonschulz(g_full_block, steps=self.group["ns_steps"])
            g_full_block = torch.sign(g_full_block)  # OLion sign operation

            # Optional scaling
            if self.group.get("use_scale", True):
                eps = self.group.get("eps", 1e-8)
                scale = (min(g_full_block.size(-2), g_full_block.size(-1))) ** 0.5 / (g_full_block.norm() + eps)
                g_full_block = g_full_block * scale
...
class SingelDeviceWork:
    ...
    def start(self):
        # Initialize momentum_buffer if needed
        ...
        update = olion_update(
            ...
            self.group.get("use_scale", True),
            self.group.get("eps", 1e-8),
        )
        self.param.mul_(1 - self.group["lr"] * self.group["weight_decay"])
        self.param.add_(update.reshape(self.param.shape), alpha=-self.group["lr"])
...
class OLion(torch.optim.Optimizer):
        ...
        for group in param_groups:
            ...
            if group["use_olion"]:
                ...
                if self.is_deepspeed_enabled and rms_scale:
                    logger.warning("rms_scale for OLion is not supported in deepspeed, setting rms_scale to False")
                    rms_scale = False
                group["rms_scale"] = rms_scale
                nesterov = group.get("nesterov", True)
                if self.is_deepspeed_enabled and not nesterov:
                    logger.warning("disabled nesterov for OLion is not supported in deepspeed, setting nesterov to True")
                    nesterov = True
    ...
    def step_deepspeed(self, closure=None):
        ...
        for group in self.param_groups:
            if group["use_olion"]:
                # for deepspeed, if we set the param.use_olion as True, the update is done in the deepspeed's framework.
